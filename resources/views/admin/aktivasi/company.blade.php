                                    ...
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td><img style="height: 100px"
                                                    src="{{ url('/') . '/mitra/logo/' . $item->logo }}"
                                                    alt="logo-perusahaan"></td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->nib }}</td>
                                            <td>
                                                <a target="_blank"
                                                    href="{{ url('/') . '/mitra/bussince_licence/' . $item->business_license }}"><svg
                                            ...
                                            <td>
                                                <div class="btn-group" role="group" aria-label="Basic example">
                                                    ...
                                                    @endphp
                                                    <form action="{{ route('company-accept') }}" method="post">
                                                        <input type="text" name="data" value="{{ $encoded }}"
                                                            hidden required>
                                                        <button type="submit"
                                                            class="btn btn-success me-3 mb-3">Setujui</button>
                                                    </form>
                                                    <form action="{{ route('company-reject') }}" method="post">
                                                        <input type="text" name="data" value="{{ $encoded }}"
                                                            hidden required>
                                                        <button type="submit"
